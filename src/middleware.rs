...
pub trait DynMiddleware<S: State>: 'static + Sync + Send {
    fn handle(&self, ctx: Context<S>, next: Next<S>) -> StatusFuture;
}

impl<S, T> DynMiddleware<S> for T
where
    S: State,
    T: 'static + Sync + Send + Fn(Context<S>, Next<S>) -> StatusFuture,
{
    fn handle(&self, ctx: Context<S>, next: Next<S>) -> StatusFuture {
        (self)(ctx, next)
    }
...
pub trait Middleware<S: State>: 'static + Sync + Send {
    ...
    fn handle(&self, ctx: Context<S>, next: Next<S>) -> Self::StatusFuture;
}
...
where
    ...
    T: 'static + Sync + Send + Fn(Context<S>, Next<S>) -> F,
{
    type StatusFuture = F;
    fn handle(&self, ctx: Context<S>, next: Next<S>) -> Self::StatusFuture {
        (self)(ctx, next)
...
pub fn make_dyn_middleware<S: State>(
    handler: impl 'static + Sync + Send + Fn(Context<S>, Next<S>) -> StatusFuture,
