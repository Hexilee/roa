...
use roa::core::{throw, Context, Error, Result, StatusCode};
use roa::preload::*;
use roa::router::Router;
use roa_diesel::{AsyncQueryDsl, WrapConnection};
...
//    let conn = ctx.state().await.get().unwrap();
//    //    spawn(async move { conn.transaction::<Option<Post>, Error, _>(|| {}) }).await?;
//    match diesel::insert_into(crate::schema::posts::table)
//        .values(&data)
//        .get_result_async::<Post>(conn)
//        .await?
//    {
    ...
    match find_post(conn, id).await? {
        ...
        Some(post) => {
            ...
            diesel::update(posts.find(id))
                ...
                .execute_async(ctx.state().await.get().unwrap())
                .await?;
