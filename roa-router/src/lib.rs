...
use radix_trie::Trie;
use roa_core::{throw, Context, DynTargetHandler, Middleware, Model, Next, Status, TargetHandler};
...
impl<M: Model> Router<M> {
    ...
    pub fn handler(self) -> Result<Box<DynTargetHandler<M, Next>>, Conflict> {
        ...
        let handler = move |ctx: Context<M>, next| {
            ...
            async move {
                let path = standardize_path(
                    &percent_decode_str(ctx.request.uri.path())
                        .decode_utf8()
                        .map_err(|err| {
                            Status::new(
